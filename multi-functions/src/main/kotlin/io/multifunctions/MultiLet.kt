 ...
 * Calls the specified function [block] with `this` value as its arguments and returns its result.
 */
inline fun <A, B, C, D, E, R> Penta<A?, B?, C?, D?, E?>.let(block: (A?, B?, C?, D?, E?) -> R): R? =
    block(first, second, third, fourth, fifth)

/**
 * Calls the specified function [block] with `this` value as its arguments and returns its result.
 */
inline fun <A, B, C, D, E, F, R> Hexa<A?, B?, C?, D?, E?, F?>.let(block: (A?, B?, C?, D?, E?, F?) -> R): R? =
    block(first, second, third, fourth, fifth, sixth)

/**
 * Calls the specified function [block] with `this` value as its arguments and returns its result.
 */
inline fun <A, B, C, D, E, F, G, R> Hepta<A?, B?, C?, D?, E?, F?, G?>.let(block: (A?, B?, C?, D?, E?, F?, G?) -> R): R? =
